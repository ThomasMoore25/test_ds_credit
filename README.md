...
2. **Парсер даты — приоритет шапки или срока оплаты.** Для invoice-подобных документов (маркеры «счёт» + «срок оплаты»/«оплата до») приоритет отдаётся сроку оплаты (фикс v0.3.0 для `invoice_002.txt`). Для остальных — дата документа (маркер «от»). Это разрешает противоречие в спецификации датасета.
...
| spec_001.txt | 1250000.0 | 2025-03-01 | 7701234567 | ООО «ТехАгро» | None | subject: в спецификации нет явного маркера «Предмет:» или шаблона поставки; тема документа — таблица номенклатуры, а не предмет оплаты |
| invoice_001.txt | 1250000.0 | 2025-03-03 | 7701234567 | ООО «ТехАгро» | None | subject: счёт не содержит явного поля «Предмет:», только таблицу с товаром; маркер «Основание: Договор...» относится к договору, а не к предмету |
...
| act_001.txt | 1250000.0 | 2025-03-24 | 7701234567 | ООО «ТехАгро» | None | subject: УПД не содержит явного маркера «Предмет:», только таблицу с товаром; предмет выводится из договора, а не из акта |
| act_002.txt | 500000.0 | 2025-04-01 | 504712345678 | ИП Смирнов В.А. | None | subject: акт выполненных работ описывает работы списком (3 пункта), но без явного маркера «Предмет:»; парсер не объединяет несколько строк в один subject |
| scan_ocr_001.txt | None | 2025-03-01 | None | None | None | amount: «l 25O OOO pyб» — буквы вместо цифр, парсер суммы не срабатывает. inn: «770l234567» — буква l вместо 1, валидатор ИНН отбрасывает. contractor: «3АО "ТexАгро"» — «3» вместо «З», паттерн ООО/АО/ПАО не срабатывает. date: восстанавливается, т.к. шаблон `\d{1,2}\.\d{1,2}\.\d{4}` толерантен к O→0 |

**Итог extract:** 5/5 ключевых полей (amount/date/inn/contractor) извлечены корректно. Поле subject извлечено в 2 из 7 файлов — это сознательное ограничение парсера: он ищет явные маркеры «Предмет:» или шаблон «передать в собственность», которых нет в spec/invoice/act. Для бизнес-кейса это допустимо — subject в этих документах можно получить из связанного договора.
